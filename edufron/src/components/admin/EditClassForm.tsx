...
const EditClassForm: React.FC<EditClassFormProps> = ({ classData, classes, onClassUpdated, onCancel }) => {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/teachers").then(r => r.json()).then(setTeachers).catch(console.error);
    fetch("http://localhost:8080/subjects").then(r => r.json()).then(setSubjects).catch(console.error);
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/classes/${classData.id}`, {
        method: "PUT",
