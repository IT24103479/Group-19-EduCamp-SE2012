...
const EditClass: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const classRes = await fetch(`http://localhost:8081/classes/${id}`);
        ...
        const teacherRes = await fetch("http://localhost:8081/teachers");
        const teacherJson = await teacherRes.json();
        setTeachers(teacherJson);

        const subjectRes = await fetch("http://localhost:8081/subjects");
