        ...
        <Card className="shadow-card">
          ...
          <CardContent>
            <Table>
              ...
              <TableBody>
                {filteredClasses.map((cls, _idx) => (
                  <TableRow key={cls.id ?? `class-${_idx}-${(cls.name || '').replace(/\s+/g, '-')}`}>
                    <TableCell>
                      <div>
                    ...
                    <TableCell>
                      {cls.subjectIds.map((sid) => {
                        ...
                        return subj ? (
                          <span key={`sub-${sid}`} className="inline-block mr-1 bg-gray-200 px-2 py-1 rounded text-xs">{subj.name}</span>
                        ) : null;
                      })}
                    </TableCell>
                    <TableCell>
