...
const Classes: React.FC = () => {
  ...
  const handleSearch = () => {
    ...
    const filtered = classes.filter((cls) => {
      if (filterType === "grade") return cls.grade.toLowerCase().includes(term);
      if (filterType === "teacher") return cls.teacher?.name.toLowerCase().includes(term);
      if (filterType === "subject")
        return cls.subjects?.some((s) => s.name.toLowerCase().includes(term));
      return false;
    });
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          ...
          <div className="relative w-full md:w-1/4">
            ...
            <select
              ...
              className="w-full pl-10 pr-4 py-3 border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent appearance-none"
            >
