...
export type Enrollment = {
  ...
  enrolledAt?: string;
  expiresAt?: string;
  // New: normalized subject information
  subjects?: Array<{ id?: number | string; name?: string }>;
  subjectNames?: string[]; // convenience array of subject names
  subjectDisplay?: string; // joined names for display
 ...
 * Also normalizes subject information (subjects array and convenience subjectNames/subjectDisplay)
 */
export function normalizeEnrollment(raw: any): Enrollment {
  ...
  // SUBJECTS: attempt to extract subject objects from multiple possible locations
  const rawSubjectsCandidates = [
    raw.classEntity?.subjects,
    raw.classEntity?.subjects_list,
    raw.classEntity?.subject,
    raw.subjects,
    raw.subject_list,
    raw.subject,
    raw.class?.subjects,
  ...
  let subjectsRaw: any[] = [];
  for (const cand of rawSubjectsCandidates) {
  ...
  // normalize subjects to {id?, name?}
  const subjects = Array.isArray(subjectsRaw)
    ? subjectsRaw.map((s: any) => {
        return {
          id: s?.id ?? s?.subject_id ?? s?.ID ?? s?.uid,
          name: s?.name ?? s?.subject_name ?? s?.title ?? (typeof s === 'string' ? s : undefined),
        };
      }).filter((s: any) => s && (s.name || s.id))
    : [];

  const subjectNames = subjects.map((s) => s.name).filter(Boolean) as string[];
  const subjectDisplay = subjectNames.length > 0 ? subjectNames.join(', ') : undefined;
  const primarySubject = subjectNames.length > 0 ? subjectNames[0] : undefined;
  ...
  return {
    ...
    subjectNames: subjectNames.length > 0 ? subjectNames : undefined,
...
export async function createEnrollment(dto: { studentId?: number; userId?: number; classId: number; paymentId?: number; subjectNames?: string[]; subjects?: any[] }) {
  const headers = {
    'Content-Type': 'application/json',
    ...getAuthHeader(),
    ...getSessionHeader(),
  ...
  if (Array.isArray(dto.subjectNames) && dto.subjectNames.length > 0 && !dto.subjects) {
    bodyDto.subjects = dto.subjectNames.map((n) => ({ name: n }));
  }

  console.debug('[ENROLLMENT SERVICE] POST', ENDPOINT, 'payload:', bodyDto);
 ...
 * Accepts optional subjectNames or subjects in dto and maps subjectNames -> subjects:{name} for the backend.
...
export async function updateEnrollment(id: string | number, dto: any) {
  ...
  const baseUrl = `${API_BASE}/api/enrollments`; // ✅ full backend URL

  // If caller provided subjectNames, convert to subjects array so backend can accept it.
  const bodyDto: any = { ...dto };
  if (Array.isArray(dto.subjectNames) && dto.subjectNames.length > 0 && !dto.subjects) {
    bodyDto.subjects = dto.subjectNames.map((n: string) => ({ name: n }));
  }
  ...
  // normalize and return the server updated enrollment
  return normalizeEnrollment(payload);
}
