...
const EditClass: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const classRes = await fetch(`http://localhost:8080/classes/${id}`);
        ...
        const teacherRes = await fetch("http://localhost:8080/teachers");
        const teacherJson = await teacherRes.json();
        setTeachers(teacherJson);

        const subjectRes = await fetch("http://localhost:8080/subjects");
